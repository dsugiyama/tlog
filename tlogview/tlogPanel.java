...
public class tlogPanel extends Panel 
    implements AdjustmentListener,MouseListener,MouseMotionListener, KeyListener
{
  tlogview tv;
  tlogDrawPanel dp;
  ...
  */
  ...
  public void moveSlider(boolean isHorizon, boolean isPositive){
      if(isHorizon){
          if(isPositive){
              base_time += 10.0/scale;
          }else{
  ...
  public void mouseReleased(MouseEvent e){
    ...
    {
    	//dp.repaint();
    	tv.setDuration(XtoT(x2)-XtoT(x1));
    }break;
    }
  }
...
class PeNumPanel extends Panel {
  ...
  public void paint(Graphics g){
    ...
    for(int pe = 0; pe < log.npe; pe++){
      ...
      Vector v = log.logData[pe];
      tlogData d = (tlogData)v.firstElement();
      int arg2 = d.getArg2();
      int pr, tr;
      pr = pe / arg2;
      tr = pe % arg2;

      g.drawString(" "+pr+" "+tr,5,y);
//      g.drawString(" "+pe,5,y);
    }
    // g.translate(0,-parent.vOffset);
  }
